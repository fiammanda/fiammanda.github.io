---
title: 计算机 10% 的 OlivaDice 免费部署指北
...
> 先运行一次OlivOS，然后按 `Ctrl+D` 停止运行程序，即可获得初始配置文件 `account.json`，参照如下注释修改。
...
1. 打开 `/home/OlivOS/conf/basic.json`，在约 100 行找到 `OlivOS_flask_post_rx` 一段，记下 `port` 值 ❹（当前版本默认为 `55001`）
```json
...
2. 打开 `/home/OlivOS/conf/account.json`，将以下内容复制进去，按注释修改并记下标号处
```json
{
	"account": [
		{
...
3. 打开 `/home/OlivOS/lib/config.yml`，查看/修改如下内容：

  1. 第 4 行 `uin`，改为机器人 QQ
```yml
account: # 账号相关
  uin: 123456789 # QQ账号 ←改这里
```

  2. 第 62 行左右 `access-token`，必须与 `account.json` 中的 `access-token` 保持一致
```yml
# 默认中间件锚点
...
```
  ...
  3. 第 97 行左右 `port`，必须与 `account.json` 中的 `port` 保持一致（当前版本默认为 `57000`）；下一行 `timeout` 值改为 `60`
```yml
  - http: # HTTP 通信设置
...
```
  ...
  4. 第 105 行左右，建议直接复制粘贴，必须保证缩进正确，端口 `55001` 与 `basic.json` 中的 `port` 保持一致
```yml
...
现在把 WinSCP 丢一边（可以关但是可能先别）并取回 PuTTY。
